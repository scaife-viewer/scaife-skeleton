<template>
  <div class="homer-source-selector">
    <button
      class="btn-book"
      @click.prevent="selectBook(index)"
      v-for="(bookLength, index) in books"
      :key="index"
      :class="{ active: selectedBook === index + 1 }"
    >
      {{ index + 1 }}
    </button>
  </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      selectedUrn() {
        return `${URN}:${this.selectedBook}.1-${this.selectedBook}.${
          BOOK_LENGTHS[this.selectedBook - 1]
        }`;
      },
      books() {
        return BOOK_LENGTHS;
